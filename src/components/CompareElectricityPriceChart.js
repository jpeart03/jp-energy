...
import { getElectricityRetailPrices } from "../api/eia";
import { Card, CardContent, Typography, Link, Fade } from "@material-ui/core";
import {
  LineChart,
  Line,
  CartesianGrid,
...
import { getRandomColor } from "../data/materialColors.js";

export const CompareElectricityPriceChart = ({ stateAbbr, chartTitle }) => {
  const [electricityRetailPrice, setElectricityRetailPrice] = useState([]);
  const [errorMessage, setErrorMessage] = useState("");

  useEffect(() => {
    const getChartData = async () => {
      const price = await getElectricityRetailPrices(stateAbbr);
      if (price.error) {
  ...
  return (
    <Fade in={!!(electricityRetailPrice.length > 0 || errorMessage)}>
